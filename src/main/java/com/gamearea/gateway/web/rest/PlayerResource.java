...
/**
 * REST controller for managing {@link com.gamearea.gateway.domain.Player}.
 */
@RestController
@RequestMapping("/api")
public class PlayerResource {
     ...
     *         body the new player, or with status {@code 400 (Bad Request)} if the
     *         player has already an ID.
    ...
    public ResponseEntity<Player> createPlayer(@RequestBody Player player) throws URISyntaxException {
        ...
        return ResponseEntity
                .created(new URI("/api/players/" + result.getId())).headers(HeaderUtil
     ...
     *         the updated player, or with status {@code 400 (Bad Request)} if the
     *         player is not valid, or with status
     *         {@code 500 (Internal Server Error)} if the player couldn't be
     *         updated.
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PutMapping("/players")
    ...
    /**
     * {@code GET  /players/check/:name} : get the player by name or create it.
     *
